...
5. 每个镜头先返回动作起点、关键动作、可见结果三张故事板画格和对应干净参考帧。镜头表、分镜带和动态预演会同步选中同一镜头。
6. 不满意时可只重生当前画格、重生当前整镜或切换历史候选；修改后必须重新批准对应画格/镜头和动态预演。
7. 全部画格、全部镜头、整板和动态预演确认后，才会显示最终额度确认并进入真实生产。
