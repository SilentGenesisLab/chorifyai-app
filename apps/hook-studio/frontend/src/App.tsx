...
function Admin({ session, onLogout }: { session: Session; onLogout: () => void }) {
  ...
  const load = useCallback(async () => { setLoading(true); try { const data = await api.admin(); const raw = asRecord(data); const healthData = asRecord(raw.health); setClients((Array.isArray(raw.clients) ? raw.clients : []).map(x => { const c = asRecord(x); return { id: String(c.id || c.code_id), name: String(c.name || c.client_name), enabled: Boolean(c.enabled), videoUsed: n(c.video_used), videoLimit: n(c.video_limit ?? c.daily_video_limit, 100), downloads: n(c.downloads) } })); const usageSource = Array.isArray(raw.usage) ? asRecord(raw.usage[0]) : raw.usage; setUsage(normalizeUsage(usageSource)); setHealth(String(healthData.database || raw.health || '正常')); setBackup(String(healthData.backup || raw.backup || '暂无记录')); setError('') } catch (e) { setError(e instanceof ApiError ? e.message : '管理数据加载失败') } finally { setLoading(false) } }, [])
  useEffect(() => { load() }, [load])
