...
function Admin({ session, onLogout }: { session: Session; onLogout: () => void }) {
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main">
      ...
      <section className="admin-table-section data-assets"><header><div><h2>训练数据资产</h2><p>对话、附件、镜头、审批和结果均已结构化入库。</p></div><FileText /></header><div className="data-counts"><span>对话<b>{numberValue(tables.conversations)}</b></span><span>消息<b>{numberValue(tables.messages)}</b></span><span>素材<b>{numberValue(tables.assets)}</b></span><span>任务<b>{numberValue(tables.task_runs)}</b></span><span>镜头<b>{numberValue(tables.storyboard_shots)}</b></span><span>分镜画格<b>{numberValue(tables.storyboard_panels)}</b></span><span>审批<b>{numberValue(tables.approval_decisions)}</b></span><span>能力运行<b>{numberValue(tables.skill_runs)}</b></span><span>训练样本<b>{numberValue(tables.training_examples)}</b></span></div><div className="data-actions"><a href={api.trainingExportUrl()}><Download />导出训练 JSONL</a><a href={api.eventsExportUrl()}><Download />导出事件 JSONL</a></div></section>
    </main>
  </div>
}
